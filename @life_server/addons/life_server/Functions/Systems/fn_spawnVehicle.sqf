...
*/
private["_vid","_sp","_pid","_query","_sql","_vehicle","_nearVehicles","_name","_side","_tickTime","_dir","_vehicleWithSirenLight"];
...
_vehicleWithSirenLight = ["B_Truck_01_transport_F","B_Truck_01_covered_F","C_Van_01_box_F","C_Offroad_01_F","B_MRAP_01_F","B_MRAP_01_hmg_F","C_SUV_01_F","O_MRAP_02_F","O_MRAP_02_hmg_F","B_MRAP_01_hmg_F","B_APC_Wheeled_01_cannon_F", "I_MRAP_03_F","I_MRAP_03_hmg_F","I_Heli_light_03_unarmed_F","B_Heli_Transport_01_F","B_Heli_Light_01_F","C_Hatchback_01_sport_F"];
...
if((_vInfo select 1) == "cop" && (_vInfo select 2) in  _vehicleWithSirenLight) then
{
	[[_vehicle,"cop_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};

if((_vInfo select 1) == "med" && (_vInfo select 2) in _vehicleWithSirenLight) then
{
	[[_vehicle,"med_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};

if((_vInfo select 1) == "adac" && (_vInfo select 2) in _vehicleWithSirenLight) then
{
...
_center_of_mass = getCenterOfMass _vehicle;
_masse = getMass _vehicle;
diag_log format["FAHRZEUGMASSE_CENTER: %1", _center_of_mass];
diag_log format["FAHRZEUGMASSE: %1", _masse];

if ((_vInfo select 2) in ["C_SUV_01_F","C_Hatchback_01_sport_F"] ) then {

_vehicle setMass (_masse + 1000);


// format of array = [-0.000676461,-0.343985,-0.589581] 
/*_center_of_mass = getCenterOfMass _vehicle;

_x_axis = (_center_of_mass select 0);
_y_axis = (_center_of_mass select 1);
_z_axis = (_center_of_mass select 2);

_x_axis = _x_axis - 4;
_y_axis = _y_axis - 4;
_z_axis = _z_axis - 4;

_vehicle setCenterOfMass [_x_axis, _y_axis, _z_axis];
*/
_masse = getMass _vehicle; 
