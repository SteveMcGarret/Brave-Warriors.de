	...
	if(player distance _x < 5 && isPlayer _x && _x != player) then {
		...
		if(!_visible) then {
			...
			_width = 0.85;
				...
				{
					case (1) : {_name = format["Recruit | %1", name _x];_icon = "";};
					case (2) : {_name = format["Officer | %1", name _x];_icon = "";};
					case (3) : {_name = format["Sergeant | %1", name _x];_icon = "";};
					case (4) : {_name = format["Lieutenant| %1", name _x];_icon = "";};
					case (5) : {_name = format["SEK | %1", name _x];_icon = "";};
					case (6) : {_name = format["MEK | %1", name _x];_icon = "";};
					case (7) : {_name = format["Chief | %1", name _x];_icon = "";};
					default {_name = name _x; _icon = ""; _width = 0; _height = 0;}
				};
                if (_x getVariable["rank", 0] > 0) then
				{
					_icon = [((_x getVariable["rank", 0]) - 1),"texture"] call BIS_fnc_rankParams;
				};
